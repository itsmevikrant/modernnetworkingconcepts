...
int create_router_sock()
{
        ...
        control_addr.sin_addr.s_addr = htonl(INADDR_ANY);
        printf("HOUX 6\n");
        control_addr.sin_port = htons(myinfo.myRport);
        printf("HOUX 7\n");
        if(bind(sock, (struct sockaddr *)&control_addr, sizeof(control_addr)) < 0)
        {
                ERROR("ERROR cannot be bind\n");
                exit(1);
        }
        printf("\n BIND: SOCKET for Sending / Receiving ROUTING Updates on Port --> %d \n", myinfo.myRport);

        /*  if(listen(sock, 5) < 0)
              ERROR("listen() failed");
           printf("\n LISTEN: SOCKET for Sending / Receiving ROUTING Updates on Port --> %d \n", myinfo.myRport);
...
bool control_recv_hook(int sock_index)
{
        ...
        case 2: initial_routing_table(sock_index);
                break;
